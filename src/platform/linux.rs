        ...
        memory_stats()
            ...
            .map(|meminfo| {
                Memory {
                    ...
                    free: ByteSize::b(meminfo.get("MemFree").map(|x| x.as_u64()).unwrap_or_default()
                     +
                        meminfo.get("Buffers").map(|x| x.as_u64()).unwrap_or_default() +
                        meminfo.get("Cached").map(|x| x.as_u64()).unwrap_or_default(
                        ) +
                        meminfo.get("SReclaimable").map(|x| x.as_u64()).unwrap_or_default(
                        ) -
                        meminfo.get("Shmem").map(|x| x.as_u64()).unwrap_or_default(
                        )),
